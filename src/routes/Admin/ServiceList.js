...
const ServiceList = () => {
    ...
    const { data: orders, isLoading, refetch } = useQuery('orders', () => fetch('https://creative-agency-ca-server.onrender.com/orders').then(res => res.json()));

    const handleState = async (state, id) => {
        const { data } = await axios.put(`https://creative-agency-ca-server.onrender.com/order/${id}`, { state: state });
        console.log(data);
        refetch();
    };
