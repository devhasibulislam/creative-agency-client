...
const MakeAdmin = () => {
    ...
    const handleMakeAdmin = async (event) => {
        ...
        const { data } = await axios.put(`https://creative-agency-server.vercel.app/email/${customerEmail}`, { role: 'admin' });
    ...
    const { data: emails, isLoading, refetch } = useQuery('emails', () => fetch('https://creative-agency-server.vercel.app/emails').then(res => res.json()));
